...
DownstreamKeyer::DownstreamKeyer(int channel, QString name, obs_view_t *v, obs_canvas_t *c, get_transitions_callback_t gt,
	...
	: outputChannel(channel),
	  ...
	  overrideTransitionDuration(300),
	  hideAfter(0),
	  view(v),
	  canvas(c),
	  get_transitions(gt),
	  get_transitions_data(gtd)
{
	...
	stateWatchdog.setInterval(1000);
	connect(&stateWatchdog, &QTimer::timeout, this, &DownstreamKeyer::ensure_desired_state);
	stateWatchdog.start();

	sourceAnimationTimer.setInterval(16);
	sourceAnimationTimer.setTimerType(Qt::PreciseTimer);
	connect(&sourceAnimationTimer, &QTimer::timeout, this, &DownstreamKeyer::source_animation_tick);
}
...
void DownstreamKeyer::on_actionRemoveScene_triggered()
{
	...
	if (!item)
		return;
	sourceAnimations.erase(QT_TO_UTF8(item->text()));
	scenesList->removeItemWidget(item);
	obs_hotkey_pair_unregister(item->data(Qt::UserRole).toUInt());
	delete item;
}
...
void DownstreamKeyer::apply_source(obs_source_t *const newSource)
{
	...
	if (exitAnimationInProgress) {
		if (!newSource)
			return;
		cancel_source_animation(true);
	}

	obs_source_t *current = view     ? obs_view_get_source(view, outputChannel)
			       : canvas ? obs_canvas_get_channel(canvas, outputChannel)
					: obs_get_output_source(outputChannel);
	obs_source_t *currentScene = current;
	if (current && obs_source_get_type(current) == OBS_SOURCE_TYPE_TRANSITION) {
		currentScene = obs_transition_get_active_source(current);
		obs_source_release(current);
	}

	if (currentScene == newSource) {
		obs_source_release(currentScene);
		return;
	}

	if (!newSource && currentScene) {
		if (start_source_animation(currentScene, true)) {
			obs_source_release(currentScene);
			return;
		}
	} else if (newSource) {
		start_source_animation(newSource, false);
	}

	obs_source_release(currentScene);
	apply_source_immediate(newSource);
}

void DownstreamKeyer::apply_source_immediate(obs_source_t *const newSource, bool skipTransition)
{
	...
	} else if (prevSource && !newSource && hideTransition && !skipTransition) {
...
void DownstreamKeyer::on_scenesList_itemSelectionChanged()
{
	...
	const auto selected = scenesList->selectedItems();
	desiredScene = selected.count() ? selected.constFirst()->text() : QString();
	temporarilySuppressed = false;

	apply_selected_source();
}

void DownstreamKeyer::ensure_desired_state()
{
	if (temporarilySuppressed || exitAnimationInProgress)
		return;

	obs_source_t *actual = view     ? obs_view_get_source(view, outputChannel)
			       : canvas ? obs_canvas_get_channel(canvas, outputChannel)
					: obs_get_output_source(outputChannel);
	obs_source_t *actualScene = actual;
	if (actual && obs_source_get_type(actual) == OBS_SOURCE_TYPE_TRANSITION) {
		actualScene = obs_transition_get_active_source(actual);
		obs_source_release(actual);
	}

	const QString actualName = actualScene ? QT_UTF8(obs_source_get_name(actualScene)) : QString();
	obs_source_release(actualScene);
	if (actualName == desiredScene)
		return;

	obs_source_t *desired = desiredScene.isEmpty()
					? nullptr
					: (canvas ? obs_canvas_get_source_by_name(canvas, QT_TO_UTF8(desiredScene))
						  : obs_get_source_by_name(QT_TO_UTF8(desiredScene)));
	if (!desiredScene.isEmpty() && !desired)
		return;

	blog(LOG_WARNING, "[KPLive Ads Keyer] restoring desired state '%s' on channel %d",
	     desiredScene.isEmpty() ? "off" : QT_TO_UTF8(desiredScene), outputChannel);
	apply_source(desired);
	obs_source_release(desired);
}
...
void DownstreamKeyer::Save(obs_data_t *data)
{
	...
	obs_data_set_string(data, "scene", QT_TO_UTF8(desiredScene));
	obs_data_set_bool(data, "desired_active", !desiredScene.isEmpty());
	obs_data_array_release(sceneArray);

	obs_data_array_t *animationArray = obs_data_array_create();
	for (const auto &[sceneName, config] : sourceAnimations) {
		auto animationData = obs_data_create();
		obs_data_set_string(animationData, "scene", sceneName.c_str());
		obs_data_set_bool(animationData, "enabled", config.enabled);
		obs_data_set_string(animationData, "source", config.sourceName.c_str());
		obs_data_set_int(animationData, "show_preset", (int)config.showPreset);
		obs_data_set_int(animationData, "hide_preset", (int)config.hidePreset);
		obs_data_set_int(animationData, "show_duration", config.showDuration);
		obs_data_set_int(animationData, "hide_duration", config.hideDuration);
		obs_data_set_int(animationData, "easing", (int)config.easing);
		obs_data_array_push_back(animationArray, animationData);
		obs_data_release(animationData);
	}
	obs_data_set_array(data, "source_animations", animationArray);
	obs_data_array_release(animationArray);
...
void DownstreamKeyer::Load(obs_data_t *data)
{
	...
	auto animationArray = obs_data_get_array(data, "source_animations");
	if (animationArray) {
		const auto animationCount = obs_data_array_count(animationArray);
		for (size_t i = 0; i < animationCount; i++) {
			auto animationData = obs_data_array_item(animationArray, i);
			const char *animationScene = obs_data_get_string(animationData, "scene");
			if (animationScene && *animationScene) {
				SourceAnimationConfig config;
				config.enabled = obs_data_get_bool(animationData, "enabled");
				config.sourceName = obs_data_get_string(animationData, "source");
				config.showPreset = (SourceAnimationPreset)obs_data_get_int(animationData, "show_preset");
				config.hidePreset = (SourceAnimationPreset)obs_data_get_int(animationData, "hide_preset");
				config.showDuration = (int)obs_data_get_int(animationData, "show_duration");
				config.hideDuration = (int)obs_data_get_int(animationData, "hide_duration");
				config.easing = (SourceAnimationEasing)obs_data_get_int(animationData, "easing");
				if (config.showDuration <= 0)
					config.showDuration = 600;
				if (config.hideDuration <= 0)
					config.hideDuration = 500;
				sourceAnimations[animationScene] = config;
			}
			obs_data_release(animationData);
		}
		obs_data_array_release(animationArray);
...
void DownstreamKeyer::source_rename(void *data, calldata_t *calldata)
{
	...
	const auto animation = downstreamKeyer->sourceAnimations.find(QT_TO_UTF8(prevName));
	if (animation != downstreamKeyer->sourceAnimations.end()) {
		auto config = animation->second;
		downstreamKeyer->sourceAnimations.erase(animation);
		downstreamKeyer->sourceAnimations[QT_TO_UTF8(newName)] = config;
	}
	for (auto &entry : downstreamKeyer->sourceAnimations) {
		auto &config = entry.second;
		if (config.sourceName == QT_TO_UTF8(prevName))
			config.sourceName = QT_TO_UTF8(newName);
	}
}
...
void DownstreamKeyer::source_remove(void *data, calldata_t *calldata)
{
	...
	downstreamKeyer->sourceAnimations.erase(QT_TO_UTF8(name));
	for (auto &entry : downstreamKeyer->sourceAnimations) {
		auto &config = entry.second;
		if (config.sourceName == QT_TO_UTF8(name)) {
			config.enabled = false;
			config.sourceName.clear();
		}
	}
