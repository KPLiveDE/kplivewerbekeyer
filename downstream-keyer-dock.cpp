...
OBS_DECLARE_MODULE()
OBS_MODULE_AUTHOR("KPLive / Exeldro");
OBS_MODULE_USE_DEFAULT_LOCALE("ads-keyer", "en-US")
...
{
	blog(LOG_INFO, "[KPLive Ads Keyer] loaded version %s", PROJECT_VERSION);
	...
	obs_frontend_add_dock_by_id("KPLiveAdsKeyerDock", obs_module_text("DownstreamKeyer"), dsk);
	_dsks[""] = dsk;
	obs_frontend_pop_ui_translation();
...
{
	vendor = obs_websocket_register_vendor("kplive-ads-keyer");
	if (!vendor)
		return;
...
void obs_module_unload()
{
	...
	_dsks.clear();
	obs_frontend_remove_dock("KPLiveAdsKeyerDock");
...
void DownstreamKeyerDock::ConfigClicked()
{
	...
	a = popup.addAction(QT_UTF8(obs_module_text("SourceAnimations")));
	connect(a, &QAction::triggered, [this]() {
		auto w = dynamic_cast<DownstreamKeyer *>(tabs->currentWidget());
		if (w)
			w->ConfigureSourceAnimation(this);
	});
